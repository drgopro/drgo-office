...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .logo { font-size:13px; font-weight:700; color:var(--accent); letter-spacing:0.15em; text-decoration:none; }
        ...
        .day-cell { background:var(--bg); min-height:100px; padding:6px; cursor:pointer; transition:background 0.1s; position:relative; }
        .day-cell:hover { background:var(--surface2); }
        .day-cell.other-month { opacity:0.35; }
        .day-cell.today .day-num { background:var(--accent); color:#1a1207; border-radius:50%; width:22px; height:22px; display:flex; align-items:center; justify-content:center; }
        ...
        /* 이벤트 칩 */
        .event-chip { font-size:11px; padding:2px 6px; border-radius:4px; margin-bottom:2px; cursor:pointer; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:#fff; }
        ...
        .event-chip.color-red { background:var(--red); }
        .event-chip.color-green { background:var(--green); color:#1a1207; }
        .event-chip.color-purple { background:var(--purple); }
        .event-chip.color-holiday { background:var(--holiday); }
        ...
        /* 유저 정보 */
        .user-menu { display:flex; align-items:center; gap:10px; font-size:12px; color:var(--text-muted); }
        .user-menu span { color:var(--text); }
        .logout-form button { background:none; border:1px solid var(--border); color:var(--text-muted); padding:5px 10px; border-radius:6px; font-size:11px; cursor:pointer; }
    </style>
</head>
<body>
...
<div class="header">
    <div class="header-left">
        <a href="/" class="logo">🟢 DRGO OFFICE</a>
        <div class="month-nav">
...
const TYPE_LABELS = { gold:'방문의뢰', teal:'원격/방송룸', blue:'사내업무', red:'휴가/개인', green:'촬영/스튜디오', purple:'미팅/내방', holiday:'공휴일' };
...
function renderCalendar() {
    ...
    for (let i = firstDay - 1; i >= 0; i--) {
        cells.push({ date: prevLastDate - i, month: 'prev', full: null });
    }
    // 이번달
    ...
    for (let d = 1; d <= remaining; d++) {
        cells.push({ date: d, month: 'next', full: null });
    }
    ...
    cells.forEach((cell, idx) => {
        ...
        if (cell.full) {
            ...
            const dayEvents = events.filter(ev =>
                ev.start_date <= cell.full && (ev.end_date || ev.start_date) >= cell.full
            );
            const MAX = 3;
            ...
            if (dayEvents.length > MAX) {
                ...
                more.textContent = `+${dayEvents.length - MAX}개 더`;
