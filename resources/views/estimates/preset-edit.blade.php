...
function buildCatTabs() {
    ...
    catData.forEach(c => {
        tabs.innerHTML += `<button class="cat-tab" onclick="filterCat(${c.id})">${_escE(c.name)}</button>`;
    });
}
...
function getCatDescendants(id) {
    let ids = [id];
    catData.forEach(c => { if (c.id === id) { ids.push(c.id); (c.children||[]).forEach(c2 => { ids.push(c2.id); (c2.children||[]).forEach(c3 => { ids.push(c3.id); (c3.children||[]).forEach(c4 => ids.push(c4.id)); }); }); } });
    return [...new Set(ids)];
}
