...
@include('partials.phone-format')
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* 직접발송(사무실 발송) — 주문완료(초록)와 시각 구분되는 주황 계열 */
        .btn-order.direct { border-color:#e0a04a; color:#d78a2e; }
        .btn-order.direct:hover { background:#fdf3e5; }
        .name-ordered.direct { background:#fdeeda; }
        .name-ordered.mixed { background:#dce8f7; } /* 세트 구성품 주문완료+직접발송 혼합 — 파랑 */
        .office-ship-badge { font-size:10.5px; color:#d78a2e; border:1px solid #e0a04a; border-radius:3px; padding:0 5px; }
...
<div class="panel-right">
    <div class="panel-right-header">
        ...
        <span style="display:flex; gap:6px; align-items:center; margin-left:auto; margin-right:8px;">
            ...
            @elseif($estimate->status === 'quote_cancelled')
                <span style="font-size:12px; padding:5px 12px; border-radius:6px; background:var(--surface2); color:var(--text-muted); font-weight:700;">견적 취소됨 — 진행이 무산된 견적서 (상태 변경으로 되살릴 수 있음)</span>
        ...
        <select id="estStatus" style="background:var(--navy); border:1px solid var(--navy); border-radius:8px; padding:6px 12px; color:#fff; font-size:12px; font-weight:700; outline:none; cursor:pointer;">
            ...
            <option value="quote_cancelled" {{ $estimate->status === 'quote_cancelled' ? 'selected' : '' }}>견적 취소</option>
...
function renderCart() {
    ...
    order.forEach((cat, gIdx) => {
        ...
        items.forEach(item => {
            ...
            let nameCls = '', nameTitle = '';
            if (item.ordered) {
                nameCls = 'name-ordered' + (item.purchase_source === '사무실 발송' ? ' direct' : '');
                nameTitle = item.purchase_source === '사무실 발송' ? '직접발송 (사무실 발송)' : '주문완료';
            } else if ((item.bundle_items || []).length && item.bundle_items.every(b => b.ordered)) {
                const directs = item.bundle_items.filter(b => b.source === '사무실 발송').length;
                if (directs === item.bundle_items.length) { nameCls = 'name-ordered direct'; nameTitle = '세트 구성품 전체 직접발송'; }
                else if (directs === 0) { nameCls = 'name-ordered'; nameTitle = '세트 구성품 전체 주문완료'; }
                else { nameCls = 'name-ordered mixed'; nameTitle = '세트 구성품 전체 처리 — 주문완료·직접발송 혼합'; }
            }
            // 주문완료 항목의 구매처/발송처 — 주문 내역에서 기입한 값을 제품명 아래 작게 (내부 화면 전용,
            // 출력물·의뢰자 견적서 미노출). 주문/배송 뷰의 '사무실 발송' 배지와 중복되는 경우만 생략
            const srcLine = item.ordered && item.purchase_source && !(orderMode && item.purchase_source === '사무실 발송')
                ? `<div style="font-size:11.5px; color:var(--text-muted); font-weight:400; margin-top:2px;">${item.purchase_source === '사무실 발송' ? '발송처' : '구매처'}: ${_escE(item.purchase_source)}</div>`
                : '';
